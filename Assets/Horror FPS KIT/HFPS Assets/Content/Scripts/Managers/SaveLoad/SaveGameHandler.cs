...
public class SaveGameHandler : MonoBehaviour {
    ...
    async void SaveGame(bool betweenScenes)
    {
        ...
        if (!betweenScenes)
        {
            if (Directory.Exists(filepath))
            {
                ...
                if (fi.Length > 0)
                {
                    ...
                    FileStream file = new FileStream(JsonManager.GetCurrentPath() + SaveName + ".sav", FileMode.Create);
                    await Task.Run(() => JsonManager.SerializeJsonDataAsync(file));
                }
                else
                {
                    lastSave = "Save0.sav";
                    FileStream file = new FileStream(JsonManager.GetCurrentPath() + "Save0.sav", FileMode.Create);
                    await Task.Run(() => JsonManager.SerializeJsonDataAsync(file));
                }
            }
            else
        ...
        {
            FileStream file = new FileStream(JsonManager.GetFilePath(FilePath.GameDataPath) + "_nextSceneData.dat", FileMode.Create);
