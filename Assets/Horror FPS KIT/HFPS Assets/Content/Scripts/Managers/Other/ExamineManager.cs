...
public class ExamineManager : MonoBehaviour {
	...
	void FirstPhase()
	{
		if(objectRaycast.tag == TagExamine || objectRaycast.tag == TagExaminePaper)
		{
            ...
            if (isPaper)
            {
                ...
                objectHeld.transform.rotation = Quaternion.LookRotation(Camera.main.transform.forward, Camera.main.transform.up) * Quaternion.Euler(rotation);
            }
    ...
    void HoldObject()
    {
        ...
        objectHeld.transform.position = Vector3.Lerp(objectHeld.transform.position, nextPos, Time.deltaTime * 10);
    }
