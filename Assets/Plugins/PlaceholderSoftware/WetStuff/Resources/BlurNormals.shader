Shader "Hidden/WS_BlurNormals"
{
    ...
    v2f vert(appdata v, float2 direction)
    {
        ...
        float2 offset1 = direction * (_Source_TexelSize.x * 1.38461538);
        float2 offset2 = direction * (_Source_TexelSize.x * 3.23076923);

        o.uv = UnityStereoScreenSpaceUVAdjust(ComputeScreenPos(o.vertex), _Source_ST);
        o.samples[0].xy = o.uv + offset1;
        o.samples[0].zw = o.uv - offset1;
        o.samples[1].xy = o.uv + offset2;
        o.samples[1].zw = o.uv - offset2;
    ...
    float4 frag(v2f i) : SV_TARGET0
    {
        ...
        normal += sampleNormal(i.samples[0].xy) * depthWeight(i.samples[0].xy, depth);// * 0.31621621;
        normal += sampleNormal(i.samples[0].zw) * depthWeight(i.samples[0].zw, depth);// * 0.31621621;
        normal += sampleNormal(i.samples[1].xy) * depthWeight(i.samples[1].xy, depth);// * 0.07027027;
        normal += sampleNormal(i.samples[1].zw) * depthWeight(i.samples[1].zw, depth);// * 0.07027027;
